    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/80 backdrop-blur-xl">
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border bg-background px-6 py-4 flex flex-col gap-4">
          <a href="#classrooms" className="font-mono text-sm text-muted-foreground hover:text-primary">classrooms</a>
          <a href="#compiler" className="font-mono text-sm text-muted-foreground hover:text-primary">compiler</a>
          <a href="#features" className="font-mono text-sm text-muted-foreground hover:text-primary">features</a>
          <div className="flex gap-2">
            <button onClick={toggleTheme} className="p-2 rounded-md border border-border text-muted-foreground">
              {isDark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
            </button>
            <button className="font-mono text-sm bg-primary text-primary-foreground px-4 py-2 rounded-md flex-1">
              $ login
            </button>
          </div>
        </div>
